...
/*
  자바스크립트에서 "함수가 자바의 메서드를 포함한다"는 의미
자바스크립트에서 "함수"는 기본적으로 값으로 취급될 수 있는 독립적인 코드 블록입니다. 즉, 자바스크립트의 함수는 변수에 할당할 수 있고, 다른 함수의 인자로 전달할 수 있으며, 다른 함수의 반환값이 될 수 있습니다. 이를 통해 자바스크립트의 함수는 일반적인 객체처럼 동작할 수 있습니다.

반면, 자바에서 함수는 독립적인 개념이 아니고 메서드라고 불립니다. 자바의 메서드는 클래스 내부에 정의된 함수입니다. 자바는 함수가 독립적으로 존재할 수 없고, 항상 클래스 내부에 속하는 메서드로 존재해야 합니다.

따라서, 자바스크립트에서는 함수가 독립적으로 존재할 수 있기 때문에, 자바의 메서드를 포함할 수 있는 **개념적으로 더 넓은 의미의 "함수"**가 존재한다고 할 수 있습니다.

"자바에 함수가 있다"는 틀리다는 말의 의미
자바에는 "함수"라는 개념이 독립적으로 존재하지 않습니다. 자바에서 함수는 항상 메서드라는 이름을 가지며, 메서드는 클래스 내에서 정의되어야 합니다. 따라서 자바에는 함수가 독립적인 개념으로 존재하지 않고, 메서드만 존재한다고 할 수 있습니다.
*/
...
// == 함수 선언(정의) ==
function greet1() {
  console.log('안녕하세요! (함수 선언식)'); // 안녕하세요! (함수 선언식)
}
...
// greet2(); - Error(호이스팅 x) 변수이기 때문에 초기화하기 전에 불가능
console.log('== 함수 선언 이전 ==');
